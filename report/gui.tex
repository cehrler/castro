...
\subfloat[][Before]{\includegraphics[width=80mm]{markneighbors_before.png}}
\subfloat[][After]{\includegraphics[width=80mm]{markneighbors_after.png}}
\end{figure}
...
By setting the index type the user can choose whether to use index and similarity matrices precomputed using either TF or TF/IDF scores. 
The user can also choose whether to use the smoothed (aliased) index or the non-aliased index. The same holds for similarity matrix, it can be either aliased (precomputed on the base of aliased index) or non-aliased (precomputed on the base of non-aliased index). By setting the similarity measure it can be choosen whether to use the manhattan distance or cosine similarity measure as the method for counting similarities.
Finally, the similarities can be calculated using only NEs (default), only lexical similarity, or may manually specify the weighting of each measure individually (\scare{custom}), e.g.\ of each type of NE compared to each other and to overall lexical similarity. 
...
We implemented a modified version of Chinese Whispering algorithm. Modified algorithm is less overgenerating. It puts the document to the cluster only when there is a strong evidence for it. By increasing the "Activation threshold multiplier" and by decreasing the "Init iterations" algorithm becomes more conservative and vice versa. 
\end{description}
